...
#    I_trans = imBitConvert(I_trans*10**3, bit=16) #AU
    I_trans = imBitConvert(I_trans*10**3, bit=16, norm=True) #AU, set norm to False for tiling images
    ...
    IHsv, IHv= PolColor(I_trans, retard, azimuth_degree)
    
#    DAPI = cv2.convertScaleAbs(DAPI*20)
#    TdTomato = cv2.convertScaleAbs(TdTomato*2)
#    IFluorAbs = np.stack([DAPI+I_trans/2, I_trans/2, TdTomato+I_trans/2],axis=2)
...
#    R=retard*I_trans
    ...
    plt.imshow(imClip(I_trans, tol=1), cmap='gray')
...
#    plt.show()
    ...
    plt.subplot(2, 3, 2)
    plt.tick_params(labelbottom=False, labelleft=False)  # labels along the bottom edge are off
    plt.imshow(imClip(retard/1000, tol=1), cmap='gray')
    plt.title('Retardance(nm)')
    plt.xticks([]), plt.yticks([])

    plt.subplot(2, 3, 3)
    plt.tick_params(labelbottom=False, labelleft=False)  # labels along the bottom edge are off
    plt.imshow(imClip(polarization, tol=1), cmap='gray')
...
#    images = [I_trans, retard, azimuth_degree, IHv, IHsv, IFluorRetard]
    
    imagesTrans = [I_trans, retard, azimuth_degree, IHv, IHsv] #trasmission channels
...
def PolColor(I_trans, retard, azimuth):
#    retard = imBitConvert(retard,bit = 8)
#    retard = imBitConvert(retard,bit = 8)
#    retard = imadjust(retard)
#    I_trans = imadjust(I_trans)
   
#    I_trans = imBitConvert(I_trans,bit = 8)
...
#    I_trans = cv2.convertScaleAbs(I_trans, alpha=(2**8-1)/np.max(I_trans))
    ...
    I_trans = cv2.convertScaleAbs(I_trans, alpha=0.1)
...
#    retardAzi = np.stack([azimuth, retard, np.ones(retard.shape).astype(np.uint8)*255],axis=2)
    IHsv = np.stack([azimuth, retard,I_trans],axis=2)
