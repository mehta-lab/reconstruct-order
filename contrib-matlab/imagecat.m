...
% IMAGECAT(...,'samescale') displays all images on the same intensity
% scale.
...
% The last seven arguments may be 'link', 'xy', 'equal','colormap', handle
% (figure or uipanel), 'off', cell array of plot titles.
...
if(any(cellfun(@(x) strcmpi(x,'link'), varargin(postargsStart:end))))
    linkplots=true;
...
if(any(cellfun(@(x) strcmpi(x,'xy'), varargin(postargsStart:end))))
    plotxy=true;
    imageidx=imageidx(1:end-1);
else
    plotxy=false;
...
if(any(cellfun(@(x) strcmpi(x,'equal'), varargin(postargsStart:end))))
    axiseq=true;
    imageidx=imageidx(1:end-1);
else
    axiseq=false;
...
if(any(cellfun(@(x) strcmpi(x,'off'), varargin(postargsStart:end))))
    axisoff=true;
    imageidx=imageidx(1:end-1);
else
    axisoff=false;
...
if(any(cellfun(@(x) strcmpi(x,'colorbar'), varargin(postargsStart:end))))
    showcolormap=true;
    imageidx=imageidx(1:end-1);
else
    showcolormap=false;
...
if(any(cellfun(@(x) strcmpi(x,'samescale'), varargin(postargsStart:end))))
    sameScale=true;
    imageidx=imageidx(1:end-1);
else
    sameScale=false;
...
h=tight_subplot(2,numcols,[0.1 0.05],[],[],hParent);

% Remove any elements on the canvas. 
